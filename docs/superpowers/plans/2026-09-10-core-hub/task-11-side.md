...
**Files:**
- Create: `js/ui/side.js`
- Modify: `js/app.js`, `styles.css` (one rule)

**Interfaces:**
- Consumes: `weekTotal`, `goalProgress`, `milestonesOf`, `goalItems`, `history`, `dayDetail`
  (`js/schedule.js`); `formatProgress`, `formatAmount`, `parseAmount` (`js/parse.js`);
  `shortDate`, `shortWeekday` (`js/dates.js`); `SOURCE_NAMES` (`js/ui/today.js`); store methods
  `acceptSuggestion`, `dismissSuggestion`, `toggleMilestone`, `archiveMilestone`,
  `addMilestone`, `logAmount`; `ctx.ui.expandedGoals`, `ctx.ui.historyDay`,
  `ctx.openEditor({ map: 'goals', id? })`.
- Produces: `renderSide(ctx)`, which fills `#side` with up to three panels:
  - **This week** — one bar per active quota (hidden when there are none).
  - **Goals** — suggested goals first (dimmed, ✓ / ✕), then active goals with title, % and a
    bar. Click one to expand in place: target date, numeric progress plus an amount input (numeric
    goals), a milestone checklist (suggested milestones get ✓ / ✕), an add-milestone input,
    linked items, and *Edit goal*. A *+ goal* link sits in the panel heading.
  - **Last 3 weeks** — M–S heading, 21 cells shaded `lvl0`–`lvl4` by done ÷ total, each showing
    `done/total`. Today is outlined. Future days are dashed and inert. Click a day to open (or
    close) its detail underneath: ✓ / ✗ per row and `+ amount title` per logged amount.

- [ ] **Step 1: Create `js/ui/side.js`**
...
Add the import below the `today.js` import:

```js
import { renderSide } from './ui/side.js';
```

And change `render()` to:
...
- [ ] **Step 3: Add the invalid-input rule to `styles.css`**

Append:

```css
.goal-body input.invalid { border-color: var(--danger); }
```
...
- **This week**: bars for Job search and Applications, and they move when you log on the left.
- **Goals**: the Gemini goal is dimmed with ✓ / ✕. "Land an analyst role" reads 33%. Expand it:
  3 milestones (the first ticked). Ticking another → 67%. Typing a milestone plus Enter adds one.
  Job search and Applications are listed as linked. *Edit goal* does nothing yet (Task 12).
- **Last 3 weeks**: 21 cells, Monday first, today outlined, future days dashed. Clicking a past day
  shows its ✓ / ✗ list and amounts. Clicking again closes it.
- At 375px the column stacks under the list.
- No console errors.

- [ ] **Step 6: Commit**

```bash
git add js/ui/side.js js/app.js styles.css
git commit -m "Add the side column: this week, goals, last three weeks"
```
